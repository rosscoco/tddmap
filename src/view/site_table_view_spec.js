...
describe("Update site data..", function() {
    ...
    it("adds a td-status-xxxx class to the row html node", function() {
        ...
        expect(
            tableRows[0].className.indexOf("td-status-complete")
        ).to.be.above(-1);
    });
    ...
    it("removes other status classes when updating", function() {
        ...
        expect(tableRows[0].className.indexOf("status-fetching")).to.be.equal(
            -1
        );
    });
    ...
    it("changes the font-awesome <i> tag class when updating the status to pending, failed, fetching, complete", function() {
        ...
        expect(
            fontAwesomeTag.className.indexOf("fa-minus"),
            "Missing fa-minus class on status icon"
        ).to.not.be.equal(-1);
        ...
        view.update(data);
        expect(
            fontAwesomeTag.className.indexOf("fa-exclamation-triangle"),
            "Missing fa-exclamation-triangle class on status icon"
        ).to.not.be.equal(-1);

        data.status = "fetching";
        view.update(data);
        expect(
            fontAwesomeTag.className.indexOf("fa-spinner"),
            "Missing fa-spinner class on status icon"
        ).to.not.be.equal(-1);

        data.status = "failed";
        view.update(data);
        expect(
            fontAwesomeTag.className.indexOf("fa-times-circle"),
            "Missing fa-times-circle class on status icon"
        ).to.not.be.equal(-1);

        data.status = "complete";
        view.update(data);
        expect(
            fontAwesomeTag.className.indexOf("fa-check-circle"),
            "Missing fa-check-circle class on status icon"
        ).to.not.be.equal(-1);
    });
    ...
    it("only allows a status class of fetching/pending/complete/failed", function() {
        ...
        expect(
            tableRows[0].className.indexOf("status-invalidClassName")
        ).to.be.equal(-1);
    });
});
